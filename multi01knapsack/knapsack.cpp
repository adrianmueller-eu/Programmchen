...
/*
This is an edited version of the code accompanying the paper "GKNAP: A Java and
C++ package for solving the multidimensional knapsack problem"
(https://arxiv.org/abs/1908.08022).
...
The multi-constraint knapsack problem is a generalization of the 0/1 knapsack
problem. The 0/1 knapsack problem has one weight constraint. The
multi-constraint knapsack problem has m constraints and one objective function
to be maximized while all the m constraints are satisfied. This makes it much
harder to solve as compared to the 0/1 knapsack problem.

The genetic algorithm is an adaptive greedy genetic algorithm which uses the
following parameters:

        Selection: Random Selection.
        Crossover: Adaptive crossover combination of the greedy and one point
crossovers. Mutation: The mutation rate is adaptively changed from 1/10N to 10/N
in steps of 1/100N depending on the progress of the algorithm in improving upon
its population. Replacement: Generational. All offspring replace their parent
individuals.
...
const double INITIAL_POPULATION_PROB =
    0.9; // the probability with which the initial population is generated
const int GENERATIONS = 100; // number of generations to run the algorithm
const int GREEDY_CROSSOVER =
    1; // a constant to identify greedy crossover in adaptive crossover
const int ONE_POINT_CROSSOVER =
    0; // a constant to identify one point crossover in adaptive crossover
const int NEGATIVE_FITNESS =
    -100; // the fitness of an invalid individual (violating constraints)
const int SHUFFLE_TOLERANCE = 100000; // the number of attempts to identify
                                      // unique parents before shuffling
...
int runtimeCrossoverType; // populated by adaptive crossover to identify which
                          // crossover operation was performed
const int MAX_UNIQUE_ITERATIONS =
    10; // maximum iterations to spend on finding objects in local improvement

/* A list of objects sorted in non-increasing order of the lagrangian
psuedo-utility ratio. Used by localImprovement() */
...
const double INITIAL_INCREMENT = 0.01; // the value of the increment with which
                                       // the lambdas are increased or decreased
const double LAMBDA_TOLERANCE = 0.00000001; // the value of the increment at
                                            // which the calculation terminates
const int COUNT_TOLERANCE = 100; // termination criteria constant for the inner
                                 // loop of calculateLagrangianMultipliers
const int DIFF_TOLERANCE = 2;    // termination criteria constant for the inner
                                 // loop of calculateLagrangianMultipliers
double *LAMBDAS;                 // the lagrangian multipliers - populated by
                                 // calculateLagrangianMultipliers
double *SOLUTION;                // the lagrangian dual solution - used by
                                 // calculateLagrangianMultipliers
...
public:
  ...
  int nodeValue() const { return value; }
  ...
  void addZero(int index) { knapsack[index] = 0; }
  ...
  void setKnapsack(int *knap) { knapsack = knap; }

  /* Is the object represented by the index chosen? If yes, return 1. If no,
   * return 0 */
  int getValueOfIndex(int index) { return knapsack[index]; }

  /* Return the array that represents the chosen and not chosen objects in order
   */
  int *getKnapsack() { return knapsack; }
...
/* Try to randomly improve upon the passed in KNode, around the greedy
 * neighborhood */
...
/* Greedy Algorithm - Creates a vector of objects arranged in non-increasing
order of the psuedu utility ratio. This vector is used by the local improvement
function */
...
141278 // the result should be (close to) this\n");
  ...
  /* Calculate the approximate lagrangian multipliers for the pseudo-utility
   * ratio */
  calculateLagrangianMultipliers();
  ...
  /* Generate a random initial population that the genetic algorithm will
   operate on. The population is generated using the INITIAL_POPULATION_PROB
   probability
   */
  vector<KNode> population;
  ...
  /* Create a global vector GREEDY_OBJECTS for use in the local improvement
   * procedure */
  greedyAlgorithm();
...
is copied to the next generation */
    newPopulation.push_back(
        population[0].clone()); // should this be population[0].clone()?
    ...
    for (int m = 0; m < count;) {
      ...
      if (runtimeCrossoverType == GREEDY_CROSSOVER) {
        m += 1;
        if (children[0].nodeFitness() <= parents[0].nodeFitness() &&
            children[0].nodeFitness() <= parents[1].nodeFitness()) {
        ...
        if (children[1].nodeFitness() <= parents[0].nodeFitness() &&
            children[1].nodeFitness() <= parents[1].nodeFitness()) {
